...
function [ Power_AP_PSI_cell Power_STA_PSI_cell SINR_cell Tx_Rate_cell] = compute_sensed_power( wlans, num_global_states, PSI_cell,...
        path_loss_model, carrier_frequency, flag_general_logs, power_sta_from_ap, distance_ap_ap, distance_ap_sta)
    ...
    %   - nChannels:  number of channels in the system
...
%                 
                for w_aux = 1 : num_wlans
                    ...
                    if wlan_ix ~= w_aux
                        ...
                        if PSI_cell{psi_ix}(w_aux,c) % If wlan_aux transmitting in state s
                            ...
                            distance_w_sta = distance_ap_sta(wlan_ix, w_aux);
                ...
                if PSI_cell{psi_ix}(wlan_ix, c) == 1                    
                    SINR = power_sta_from_ap(wlan_ix, wlan_ix) - 3 * n_channels_used_for_tx - ...
                        pow2db(db2pow(Power_AP_PSI_cell{psi_ix}(wlan_ix,c)) + db2pow(NOISE_DBM));
                    tx_rate = compute_tx_rate_according_to_sinr(SINR, n_channels_used_for_tx); 
                else
                    SINR = -Inf;
                    tx_rate = 0;
                end 
                SINR_cell{psi_ix}(wlan_ix, c) = SINR;   
                % FRANKY: Compute the tx rate according to the SINR
                Tx_Rate_cell{psi_ix}(wlan_ix, c) = tx_rate;
