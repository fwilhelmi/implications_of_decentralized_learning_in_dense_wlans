...
function [ Q, S, S_cell, T_S, T_PSI, S_num_states ] = identify_feasible_states_and_Q( PSI_cell, Power_PSI_cell, mcs_per_wlan,...
...
%                         aux_rate = Tx_Rate_cell{origin_psi_ix}(wlan_ix, :);
%                         rate = min(aux_rate(aux_rate > 0));
%                         if isempty(rate), rate = MINIMUM_TX_RATE; end                        
%                         mu_s = rate / (PACKET_LENGTH*NUM_PACKETS_AGGREGATED);% * NUM_PACKETS_AGGREGATED * PACKET_LENGTH;

                        mu_s = mcs_rates(num_ch_wlan_s, mcs_per_wlan(wlan_ix, num_ch_wlan_s));
                        
                        
%                         disp('-------')
%                         disp(['* origin_s_ix: ' num2str(origin_s_ix)])
%                         disp(['* destination_s_ix: ' num2str(destination_s_ix)])
%                         disp(['* wlan_ix: ' num2str(wlan_ix)])
%                         disp(['* Tx_Rate_cell{origin_psi_ix}(wlan_ix, :): ' num2str(Tx_Rate_cell{origin_psi_ix}(wlan_ix, :))])
