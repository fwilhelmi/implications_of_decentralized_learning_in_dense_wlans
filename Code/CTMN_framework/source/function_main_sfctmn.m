...
function throughput = function_main_sfctmn(wlans)
    ...
    constants_ctmn        % Execute constants_script.m script to store constants in the workspace
    ...
    % FRANKY: Compute the distance between the APs and the STAs
    [distanceApAp, distanceApSta] = compute_distance_nodes(wlans);
    
    for i = 1 : num_wlans        
        for j = 1 : num_wlans
            powerRxStationFromAp(i,j) = compute_power_received(distanceApSta(i,j), ...
                wlans(i).tx_power, GAIN_TX_DEFAULT, GAIN_RX_DEFAULT, carrier_frequency, path_loss_model);        
            powerRxApFromAp(i,j) = compute_power_received(distanceApAp(i,j), ...
                wlans(i).tx_power, GAIN_TX_DEFAULT, GAIN_RX_DEFAULT, carrier_frequency, path_loss_model);        
        end
        sinrIsolation(i) = compute_sinr(powerRxStationFromAp(i,i), -Inf, NOISE_DBM);    % SINR sensed in the STA in isolation (just considering ambient noise)
    end
    
%     power_from_ap = compute_power_received(distanceApSta, TX_POWER_DEFAULT, GAIN_TX_DEFAULT,...
%         GAIN_RX_DEFAULT, carrier_frequency, path_loss_model);

    % Compute the MCS according to the SINR in isolation mode
    mcsPerWlan = compute_MCS(powerRxStationFromAp, nChannels);
    
    display_wlans(wlans, flag_display_wlans, flag_plot_wlans, flag_plot_ch_allocation, nChannels,...
        path_loss_model, carrier_frequency, sinrIsolation)
    ...
    [Power_AP_PSI_cell Power_STA_PSI_cell SINR_cell Tx_Rate_cell] = compute_sensed_power(wlans, num_global_states, PSI_cell, path_loss_model,...
        carrier_frequency, flag_general_logs, powerRxStationFromAp, distanceApAp, distanceApSta);
    display_with_flag([LOG_LVL2 'Sensed power computed!'], flag_general_logs)
    
    % DISPLAY SINR AND TX RATE FOR STATE 16
%     disp('POWER AP:')
%     Power_AP_PSI_cell{16}(:,:)
%     disp('SINR:')
%     SINR_cell{16}(:,:)
%     disp('TX RATE:')
%     Tx_Rate_cell{16}(:,:)
...
%         disp(' - SINR_cell{i}(:,:): ' )
%         disp(num2str(SINR_cell{i}(:,:)))
%         disp([' - Tx_Rate_cell{i}(:,:): '])
%         disp(num2str(Tx_Rate_cell{i}(:,:)*10^-6))
%     end
    ...
    [ Q, S, S_cell, Q_logical_S, Q_logical_PSI, S_num_states ] = identify_feasible_states_and_Q(PSI_cell, Power_AP_PSI_cell, mcsPerWlan, ...
        nChannels, wlans, dsa_policy_type, flag_logs_feasible_space);
    ...
    [prob_tx_num_channels_success, prob_tx_num_channels_unsuccess] = ...
        get_probability_tx_in_n_channels(Power_STA_PSI_cell,...
        S_cell, PSI_cell, num_wlans, nChannels, p_equilibrium, powerRxStationFromAp, wlans);%path_loss_model, distanceApSta, wlans,...
       % carrier_frequency);

    display_with_flag([LOG_LVL2 'Probability of transmitting SUCCESSFULLY in num channels (0:nChannels): '], flag_general_logs)
    display_with_flag(prob_tx_num_channels_success, flag_general_logs)
    display_with_flag([LOG_LVL2 'Probability of transmitting UNSUCCESSFULLY (i.e. packet losses) in num channels (0:nChannels): '], flag_general_logs)
    display_with_flag(prob_tx_num_channels_unsuccess, flag_general_logs)
    ...
    throughput = get_throughput(wlans, num_wlans, p_equilibrium, S_cell, ...
        PSI_cell, SINR_cell, mcsPerWlan, powerRxStationFromAp);
